...
def li(rd, value):
    return 0x38000000 | (rd << 21) | (value & 0xFFFF)
...
def b(frm, to):
    return 0x48000000 | ((to - frm) & 0x03FFFFFC)


def cmpwi(ra, value):
    return 0x2F800000 | (ra << 16) | (value & 0xFFFF)


def beq(frm, to):
    return 0x419E0000 | ((to - frm) & 0xFFFC)
...
def test_apply_patch_file_verifies_expect_first(tmp_path):
    spec = {
        "variants": {
            "NAV": {"patches": [{"addr": hex(BASE), "expect": "38600000", "bytes": "38600001"}]}
        }
    }
...
def test_apply_patch_file_refuses_a_mismatched_image(tmp_path):
    spec = {
        "variants": {
            "NAV": {"patches": [{"addr": hex(BASE), "expect": "deadbeef", "bytes": "38600001"}]}
        }
    }
    path = tmp_path / "p.json"
    path.write_text(__import__("json").dumps(spec))
...
def test_calls_are_recorded_and_really_executed():
    e = emu(
        {
            BASE: [stwu(1, -16, 1), mflr(0), stw(0, 20, 1), bl(BASE + 0xC, CALLEE), blr()],
            CALLEE: [li(3, 7), blr()],
        }
    )
...
def test_a_stub_replaces_the_callee_and_its_body_never_runs():
    e = emu({BASE: [bl(BASE, CALLEE), blr()], CALLEE: [li(3, 7), blr()]}, trace=True)
    e.stub(CALLEE, 99)
...
def test_nopping_a_branch_changes_what_is_reached():
    ...
    layout = {
        BASE: [
            ...
            li(3, 0),
            cmpwi(3, 0),
            beq(BASE + 0x14, BASE + 0x1C),  # skip the call
            bl(BASE + 0x18, CALLEE),
            blr(),
        ],
        CALLEE: [li(3, 123), blr()],
